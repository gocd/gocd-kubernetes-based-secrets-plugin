...
public class CredentialValidator implements Validator {
    ...
    public ValidationResult validate(Map<String, String> requestBody) {
        ...
        try (KubernetesClientFactory.CachedClient client = kubernetesClientFactory.client(secretConfig)) {

            try {
                Secret secret = client.get().secrets().inNamespace(secretConfig.getNamespace()).withName(secretConfig.getSecretName()).get();
                if (secret == null) {
